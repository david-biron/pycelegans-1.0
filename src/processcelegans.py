...
def process_jpgs(FileNames):
    for file_pathname in FileNames:
        if FILTER in file_pathname:
            try:
                workflow.process_frame(file_pathname,max_image,THRESH,THRESH_DIFF,NPOINTS,ANGLE_STEP_SIZE,SCRATCHDIR,DOVIZ,VIZDIR,DETMODE,WORMWIDTH,WORMLENGTH)
                print file_pathname
...
if __name__ == "__main__":
    ...
    parser.add_argument('--detmode',action='store',dest='DETMODE',required=False,default=1,
     help='detection mode, 1 = use head features / 2 = no head features')
